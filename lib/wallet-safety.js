...
// magnitude.
// The ONE grammar both helpers accept: an optional sign, digits, optionally a
// dot and more digits. Nothing else.
//
// Audit F-194 (round 2). isPositiveNumberInput used parseFloat, which parses a
// PREFIX: parseFloat('1,5') is 1, so the guard said "valid amount" and then
// pdexToPlanck's BigInt('1,5') threw — the user could not complete the
// transaction and got an exception instead of a message. Fail-closed, so no
// wrong amount was ever signed.
//
// The reason to fix it properly rather than shrug: the obvious "just strip the
// commas" patch IS a funds bug. '1,5' means 1.5 in most of Europe and 15 if you
// delete the separator, and the amount field is a plain type=number that a
// mobile WebView keyboard can populate with either convention. There is no way
// to know which the user meant, so the only safe answer is to refuse and let
// them retype. Same for '1e5', which parseFloat happily reads as 100000 and
// BigInt rejects.
//
// Rejecting is not a limitation to be relaxed later. It is the feature.
const AMOUNT_GRAMMAR = /^[+-]?\d+(\.\d+)?$/;

export function pdexToPlanck(value) {
    const s = String(value == null ? '0' : value).trim();
    if (!s) return '0';
    if (!AMOUNT_GRAMMAR.test(s)) {
        // Explicit, before BigInt gets a chance to throw something opaque.
        // Callers gate on isPositiveNumberInput first, so reaching this means a
        // NEW call site skipped the guard — say so plainly.
        throw new Error(`Not a plain decimal amount: ${JSON.stringify(s)}. `
...
export function isPositiveNumberInput(str) {
    ...
    if (s === '') return false;
    // F-194: was parseFloat, which accepts a PREFIX — '1,5' scanned as 1 and
    // '1e5xyz' as 100000 — so the guard passed values pdexToPlanck cannot
    // convert. Both helpers now share one grammar, so "the guard said yes" and
    // "the converter can do it" cannot disagree.
    if (!AMOUNT_GRAMMAR.test(s)) return false;
    const n = Number(s);
