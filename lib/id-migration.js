...
/**
 * How many rows the hash-keyed id rewrite would touch, without touching them.
 * (Audit F-182)
 *
 * The rewrite itself is one BEGIN IMMEDIATE per table, so on a database with a
 * large legacy history it holds the write lock for the whole pass — at indexer
 * boot, before the indexer may start. That is fine on a fresh install (zero
 * rows) and unacceptable on a big one, and the code had no way to tell the two
 * apart because it never asked.
 *
 * Read-only: COUNT takes no write lock, so this is safe to run at boot on any
 * size of database. It is a scan, but a single one, and the predicates are
 * character-for-character the ones migrateHashKeyedIds uses — if those ever
 * drift, the gate is sizing a different population than the migration rewrites,
 * so they are asserted equal in test/hash-id-gate.test.js.
 */
export function countHashKeyedIdCandidates(dbh) {
    const tx = dbh.prepare(`
    ...
    `).get();
    const rewards = dbh.prepare(`
        SELECT COUNT(*) AS n FROM staking_rewards
         WHERE block_hash LIKE '0x%'
           AND event_index IS NOT NULL
           AND id NOT LIKE '0x%'
    `).get();
    const transactions = Number(tx && tx.n) || 0;
    const stakingRewards = Number(rewards && rewards.n) || 0;
    return { transactions, stakingRewards, total: transactions + stakingRewards };
}
